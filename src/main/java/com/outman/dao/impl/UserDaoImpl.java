...
public class UserDaoImpl implements UserDao{
    ...
    public User findUserByPhone(String userPhone) throws Exception {
        ...
        User user = null;
        //生成key，格式可以根据业务自行修改，但是必须得唯一
        String key = Joiner.on("_").join(Arrays.asList(userPhone,userPhone));
    ...
    public Integer findOrgIdByUserId(Integer userId)throws Exception {
        ...
        Integer orgId = 0;
        orgId = userMapper.findOrgIdByUserId(userId);
        return orgId;
    }
    ...
    public Org findOrgInfoByOrgId(Integer orgId) throws Exception {
        ...
        Org org = null;
        org = userMapper.findOrgInfoByOrgId(orgId);
        return org;
    }

    public List<InviteEntity> isInvited(Integer userId) {
        List<InviteEntity> inviteEntities = userMapper.isInvited(userId);
